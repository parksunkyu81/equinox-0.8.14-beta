...
class PIDController:
  def __init__(self, k_p=0., k_i=0., k_d=0., k_f=1., pos_limit=None, neg_limit=None, rate=100, sat_limit=0.8, derivative_period=1.):
    self._k_p = k_p  # proportional gain (비례 게인)
    self._k_i = k_i  # integral gain  (적분 게인)
    self._k_d = k_d  # derivative gain (파생 게인)
    self.k_f = k_f   # feedforward gain (피드 포워드 : 앞으로의 징후를 계한에 의해 예측하고 그 정보에 기준하여 제어를 행하는 방식)
    if isinstance(self._k_p, Number):
      self._k_p = [[0], [self._k_p]]
    if isinstance(self._k_i, Number):
      self._k_i = [[0], [self._k_i]]
