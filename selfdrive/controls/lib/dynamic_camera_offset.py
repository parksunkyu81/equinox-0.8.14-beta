...
class DynamicCameraOffset:  # 다가오는 차량으로부터 멀리 유지
  def __init__(self):
  ...
  def _setup_static(self):  # these variables are static
    ...
    self._keep_offset_for = self.op_params.get('dynamic_camera_offset_time')  # 다가오는 차선을 잃은 후 몇 초
  ...
  def update(self, v_ego, active, angle_steers, lane_width_estimate, lane_width_certainty, polys, probs):
    self.camera_offset = self.op_params.get('camera_offset')  # 사용자로부터 기본 오프셋을 업데이트합니다
